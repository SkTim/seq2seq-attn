...
function train(train_data, valid_data, train_iter)
  ...
  for iter = 1, train_iter do
    decoder_clones[iter] = clone_many_times(decoder, opt.max_sent_l_targ)
    encoder_clones[iter] = clone_many_times(encoder, opt.max_sent_l_src)
  end
  -- if opt.brnn == 1 then
  ...
  for i = 1, opt.max_sent_l_src do
    if encoder_clones[iter][i].apply then
      encoder_clones[iter][i]:apply(function(m) m:setReuse() end)
      if opt.prealloc == 1 then encoder_clones[iter][i]:apply(function(m) m:setPrealloc() end) end
    end
    if opt.brnn == 1 then
  ...
  for i = 1, opt.max_sent_l_targ do
    if decoder_clones[iter][i].apply then
      decoder_clones[iter][i]:apply(function(m) m:setReuse() end)
      if opt.prealloc == 1 then decoder_clones[iter][i]:apply(function(m) m:setPrealloc() end) end
    end
  end
  end
